...
static void checkNode(TreeNode *t)
{
	switch (t->kind)
	{
		...
		case WhileStmt:
		{
			...
			//checkNode(conditionNode);
			if(conditionNode->type != Integer){
		...
		case ReturnStmt:
		{
		    ...
		    if(t->flag == 1){
				...
				//checkNode(t->child[0]);
				if(t->child[0]->type != currentScope->func->type) InvalidReturnError(t->lineno);
			}
		...
		case CallExpr:
		{
			...
			while(paramNode != NULL || argNode != NULL){
				...
				else if(paramNode != NULL && argNode == NULL){
					if(paramNode->flag == 1) break;
					else{
						InvalidFunctionCallError(t->name, t->lineno);
						break;
					}
				...
				//checkNode(paramNode);
				//checkNode(argNode);
				if(paramNode->type != argNode->type || paramNode->flag == 1){
