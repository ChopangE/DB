...
def display():
    global cameraIndex, cow2wld, check, checklist, insert, pointlist, start, animStartTime, cowwld_, pickInfo,count,first, isDrag
    ...
    if(check > 5):
        ...
        if(t > 0 and t <= 1):
            ...
            c = t_array @ b
            d = c @ np.array([pointlist[0],
                              pointlist[1],
                              pointlist[2],
                              pointlist[3]])
            setTranslation(T, d-pp.cowPickPosition)
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, check, insert, pickInfo,pointlist,first
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        ...
        elif state == GLFW_UP and first == 0:
            isDrag=H_DRAG;
            print( "Left mouse up\n");
            first = 1
