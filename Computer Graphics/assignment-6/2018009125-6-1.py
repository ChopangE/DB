...
def render():
    global gCamAng, gCamHeight, xang, zang, z2ang
    ...
    Rz2 = np.array([[np.cos(z2ang), -np.sin(z2ang), 0],
                   [np.sin(z2ang), np.cos(z2ang), 0],
                   [0,0,1]])
...
def key_callback(window, key, scancode, action, mods):
    global gCamAng, gCamHeight, zang, xang, z2ang
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2018009125-6-1', None,None)
    if not window:
        glfw.terminate()
        return
